...
*(se irá llenando a medida que avancemos por los items 4–10 del análisis)*
...
- **Pendiente de deploy:** `cd worker && wrangler deploy` cuando se apruebe el lote de
  cambios del bot.
...
| 4 | Registro explícito del consentimiento de contacto (compliance) | Sí | Pendiente |
| 5 | Estado estructurado de conversación + resumen sintetizado para el agente humano | Sí — hacer antes de duplicar | Pendiente |
| 6 | Flujo de repatriación / traslado internacional | Casi seguro N/A para FDZ | Pendiente (decisión de negocio LEGADO) |
| 7 | Conciencia de horario de atención de la guardia | Sí | Pendiente (info de operación) |
| 8 | Idempotencia por mensaje en `create_lead` / `handoff_whatsapp` | Sí | Pendiente |
| 9 | Detección de idioma dentro de la conversación | Depende (si FDZ es ES-only, menor) | Pendiente |
| 10 | "Confirma antes de actuar" — resumir y confirmar antes de `create_lead` / handoff | Sí | Pendiente |
